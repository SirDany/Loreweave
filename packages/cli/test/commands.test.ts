...
async function zipDir(dir: string, outFile: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    a.append(
      JSON.stringify({ loreweave: { kind: 'saga-export', version: 1 } }),
      {
...
describe('stripRefs', () => {
  it('replaces @type/id with display names from the index', () => {
    ...
    const text =
      '@character/bella raised her @term/skol and @character/unknown watched.';
...
describe('import buildPlan', () => {
  it('classifies files as new / conflict / unchanged via sha256', async () => {
    ...
    expect(plan.conflicts.map((c) => c.relPath)).toEqual([
      'codex/characters/b.md',
    ]);
  ...
  it('detects bundle root from single top-level folder', async () => {
    ...
    await fs.mkdir(path.join(staging, 'only-one', 'codex'), {
      recursive: true,
    });
    await fs.writeFile(
      path.join(staging, 'only-one', 'saga.yaml'),
      'x',
      'utf8'
    );
  ...
  it('round-trips a well-formed zip', async () => {
    ...
    try {
      const inner = await fs.readFile(
        path.join(extracted, 'zip-saga', 'saga.yaml'),
        'utf8'
      );
...
describe('ingest ALLOWED_EXT and stageOne', () => {
  ...
  it('stageOne copies allowed files and skips disallowed ones', async () => {
    ...
    const okFile = path.join(base, 'traces.md');
    ...
    const copied = await fs.readFile(path.join(ingest, 'traces.md'), 'utf8');
