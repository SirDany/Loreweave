...
async function buildSaga(name: string): Promise<string> {
  ...
  await writeFile(
    path.join(root, "notes/remember-aaron.md"),
...
describe("lw rename", () => {
  it("plans echo rewrites and file rename for id change", async () => {
    ...
    const plan = await buildRenamePlan(saga, "character/aaron", "aaron-stormrider");
    ...
    expect(files).toContain("notes/remember-aaron.md");
  ...
  it("applies rename: rewrites echoes, updates frontmatter id, renames file", async () => {
    ...
    const note = await fs.readFile(
      path.join(saga, "notes/remember-aaron.md"),
      "utf8",
    );
    expect(note).toContain("@character/aaron-stormrider");
  });
  ...
  it("rewrites waypoint event-field on threads", async () => {
    ...
    await applyRenamePlan(root, plan);
    const main = await fs.readFile(path.join(root, "threads/main.yaml"), "utf8");
