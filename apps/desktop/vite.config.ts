      ...
      server.middlewares.use("/lw/write", (req, res) => {
        ...
        req.on("end", async () => {
          try {
            ...
            if (typeof sagaRoot !== "string" || typeof relPath !== "string" || typeof content !== "string") {
              throw new Error("sagaRoot, relPath, content required");
            }
            const safe = safeJoin(
              path.isAbsolute(sagaRoot) ? sagaRoot : path.join(REPO_ROOT, sagaRoot),
              relPath,
            );
...
function safeJoin(root: string, rel: string): string {
  if (rel.includes("..")) throw new Error("relative path must not contain '..'");
  const normalizedRoot = path.resolve(root);
  const joined = path.resolve(normalizedRoot, rel);
  if (!joined.startsWith(normalizedRoot + path.sep) && joined !== normalizedRoot) {
    throw new Error(`path escape detected: ${joined} is outside ${normalizedRoot}`);
  }
  return joined;
}
